...
# nice to have all the figures have the same color scheme for continuity :)
lotsco_col <- "#E69512"
pencen_col <- "#D6264F"
salleu_col <- "#6D397D"
enccal_col <- "#3A5565"
...
drought_exp_clean <- drought_exp |>  # start w drought_exp object
  ...
  # as factor and fct relevel makes species name explicitly a factor
  # make sure that species_name is a factor and ordering levels
  mutate(species_name = as_factor(species_name),
         species_name = fct_relevel(species_name,
                                    ...
                                    "Grindelia camporum")) |> 
  # make sure that water_treatment is a factor, reordering the levels
  mutate(water_treatment = as_factor(water_treatment),
         water_treatment = fct_relevel(water_treatment,
                                       "Drought stressed",
                                       "Well watered")) |> 
  # selecting columns of interest in clean object
...
slice_sample(
  drought_exp_clean, # dispaying rows from object 
  n = 10 # 10 rows
)
```
...
ggplot(data = drought_exp_clean,
  ...
  labs(x = expression(Specific~leaf~area~(cm^2/g)),
       y = "Total mass (g)",
       title = "As SLA increases, total plant mass increases") #name plots after your results!! to remember what it is
```
...
ggplot(data = drought_exp_clean,
  ...
  labs(x = "Water treatment",
       y = "Total mass (g)",
       title = "Average total mass and spread are \nhigher in well watered treatment") +
  # taking out legend
...
```{r model-selection}
# use AICc from MoMin package to help choose model
AICc(
  model0,
  model1,
  model2,
  ...
  arrange(AICc) #arrange AICcs by value
```
...
summary(model4)
#reference level for water_treatment: drought stressed (bc it doesnt show up in summary)
#reference species for species_name: Lotus Scoparius (bc it doesnt show up in the summary)
#this happens bc those are the first factors you set in #2- clean data
...
This model explained 73% (adjusted R^2^) of the total plant mass.

**How do we interpret the effects of the predictors on the response variable (again, total mass in grams)?**

On average, plant masses in drought stressed conditions were 0.12 $\pm$ 0.02 less than plant masses in well watered conditions across species.
...
model4_predictions <- ggpredict(
  ...
  terms = c("species_name", "water_treatment") #naming predictors in the model
) |> 
